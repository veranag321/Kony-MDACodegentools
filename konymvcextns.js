...
BusinessController_CommandHandler = function (Command, CommandResponse) {
  function CommandHandler(commandId, commandHandlerExtension) {
    this.commandId = commandId;
    this.commandHandlerExtension = commandHandlerExtension;
    ...
    for (var i = 0; i < commandHandlers.length; i++) {
      ...
      if (commandHandlers[i].CommandHandlerExtension && commandHandlers[i].CommandHandlerExtension != null) {
        ...
        try {
          ...
          throw new Exception(ERROR_CODE_300, 'Missing Command Handler Extension.');
        }
      } else {
        var commandExtension = null;
      }
      var handler = new commandHandler(commandId, commandExtension);
...
DataModel_DBAssembler = function () {
  var assembler = {};
  /**
  ...
  assembler.fromDBJson = function (BaseModelClass, config, dbJson) {
    ...
    return new BaseModelClass(modelJson);
  };
  return assembler;
}();
DataModel_DataSource = function (QueryBuilder, constants, DBAssembler) {
  ...
  DataSource.prototype.save = function (BaseModel, config, onCompletion, accessMode) {
    ...
    for (var attribute in BaseModel.attributeMap) {
      dataObject.addField(attribute, BaseModel.attributeMap[attribute]);
    }
    var options = {};
    options[constants.DATA_OBJECT] = dataObject;
    options[constants.HEADERS_STRING] = headers;
...
PresentationController_MDABasePresenter = function () {
  ...
  MDABasePresenter.prototype.presentUserInterface = function (uiTag, context) {
  ...
  MDABaseNavigator.prototype.presentForm = function (form, uiTag, viewModel) {
    ...
    if (form == _currentForm.id) {
      ...
      try {
        ...
        throw new Exception(ERROR_CODE_300, 'Error at Navigator, Missing or Wrong UITag ');
      }
    }
  };
...
ModuleManager_MDAModule = function (MDABasePresenter, MDABaseNavigator, BusinessController, constants) {
  function MDAModule(moduleName, channel) {
    ...
    try {
      ...
      throw new Exception(ERROR_CODE_600, 'Module Config expected');
    }
    this.setupModule();
  }
    ...
    if (this.moduleConfig === undefined) {
      ...
      if (businessControllerClass === undefined || businessControllerClass === '') {
        ...
        try {
          ...
          throw new Exception(ERROR_CODE_300, 'Invalid Business Controller Class');
        }
      }
    }
    ...
    if (this.moduleConfig === undefined) {
      ...
      if (presentationControllerClass === undefined || presentationControllerClass === '') {
        ...
        try {
          ...
          throw new Exception(ERROR_CODE_300, ' Invalid Presentation Controller Class');
        }
      }
    }
      ...
      if (forms !== undefined) {
        for (var i = 0; i < forms.length; i++) {
          if (kony.mvc.registry.getControllerType(forms[i].FormName) != constants.FORM_CONTROLLER_TYPE) {
            ...
            kony.mvc.registry.remove(formName, formName);
            kony.mvc.registry.add(formName, formName, {
    ...
    if (this.moduleConfig !== undefined) {
      ...
      for (var i = 0; i < presentationExtensions.length; i++) {
        try {
          ...
          throw new Exception(ERROR_CODE_300, 'Presentation Controller Extension invalid');
        }
  ...
  return MDAModule;
}(PresentationController_MDABasePresenter, BaseNavigator_MDABaseNavigator, BusinessController_BusinessController, DataModel_constants);
...
DataModel_BaseModel = function (ModelRelation) {
  ...
  /**
   * @function add Attribute adds new attribute and value to existing model
   * @param  {attributeKey} attribute key
   * @param  {attributeValue} attribute value
   * @return
   */
  BaseModel.prototype.addAttribute = function (attributeKey, attributeValue) {
    this.attributeMap[attributeKey] = attributeValue;
  };
  /**
   * @function save saves the given model data in the backend
   * @param  {function} onCompletion success callback
...
commonUtils_ControllerGetterAPI = function () {
  function getController(formId) {
    let tempController = kony.mvc.registry.get(formId);
    let controllerType = 'kony.mvc.MDAFormController';
    let isForm = true;
    if (tempController != null) {
      let tempControllerType = kony.mvc.registry.getControllerType(formId);
      if (tempControllerType == controllerType) {
        return _kony.mvc.GetController(formId, isForm);
      } else {
        let friendlyName = kony.mvc.registry.getFriendlyName(formId);
        let controllerName = kony.mvc.registry.getControllerName(formId);
        let controllerExtName = kony.mvc.registry.getControllerExtName(formId);
